// Carga los tests del curso «Operadores en establecimientos de beneficio»
// desde el libro de preguntas aportado y validado.
//
...
// A diferencia del libro de Administración, aquí cada pregunta declara ya su
// unidad («1.1», «3.7»…), así que no hay que deducirla del número de
// diapositiva.
//
...
//
// La versión de 20 h sigue en borrador mientras faltan locuciones, de modo que
// sus bloques 3, 4 y 5 no tienen todavía ninguna unidad publicada. Sus
// preguntas se cargan igualmente y quedan sin enlazar a unidad; el enlace sólo
// alimenta la lista de partes a repasar y podrá rehacerse cuando llegue el
// audio que falta.
//
...
//
// Uso:
//   node scripts/import-beneficio-quizzes.mjs [--dry-run]
...
export const WORKBOOK =
  'Contenido Cursos/Preguntas_Establecimientos_Beneficio_5h_20h_VALIDADO.xlsx'
const BLOCKS = 5
...
async function main() {
  ...
  for (const version of versions) {
    ...
    for (const module of modules) {
      ...
      // banco, que es como está cargado el resto del campus. Y si la unidad
      // todavía no está publicada —bloques sin locución de la versión en
      // borrador— la pregunta queda sin enlazar.
      const linked = new Set()
      const rows = blockQuestions.map((question) => {
        const segmentId = segmentByCode.get(question.code)
        const first = Boolean(segmentId) && !linked.has(question.code)
      ...
      const { data: bank, error: bankError } = await supabase
        ...
        .insert({
          ...
          title: `Evaluación aportada · Establecimientos de beneficio ${duration} h · Bloque ${block} · 2026-09-11`,
        })
        .select('id')
        .single()
