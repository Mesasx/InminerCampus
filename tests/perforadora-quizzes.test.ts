...
// Los tests de Perforadora se cargaron desde el libro de preguntas aportado.
...
const migrationUrl = new URL(
  '../supabase/migrations/20260911160000_tests_operador_perforadora.sql',
  import.meta.url,
)
const importerUrl = new URL(
  '../scripts/import-perforadora-quizzes.mjs',
  import.meta.url,
)
...
test('la migración trae las 125 preguntas repartidas como exige el libro', () => {
  const rows = [...migration.matchAll(/^ {2}\('PERF-(\d+)H-B(\d)-Q(\d+)'/gm)].map(
    ([, duration, block]) => ({
...
test('las dos modalidades están publicadas, así que ninguna pregunta se queda sin unidad', () => {
  // A diferencia de Beneficio, aquí las cincuenta unidades de cada modalidad
  // están publicadas: si alguna faltara, la carga se detiene en vez de dejar
  // preguntas huérfanas.
  assert.match(importer, /No se encuentra la unidad \$\{question\.code\}/)
  assert.match(migration, /raise exception 'No se encuentra la unidad %/)
})

test('cada pregunta apunta a una unidad real de su propio bloque', () => {
  // Cinco bloques de diez unidades en ambas modalidades.
  assert.match(importer, /const BLOCKS = 5/)

  const rows = [
    ...migration.matchAll(
      /^ {2}\('PERF-\d+H-B(\d)-Q\d+', \d+, (\d), \d+, '(\d+)\.(\d+)'/gm,
    ),
  ]
  assert.equal(rows.length, 125)
